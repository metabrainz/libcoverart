...
#ifndef _COVERARTARCHIVE_HTTP_FETCH_
#define _COVERARTARCHIVE_HTTP_FETCH_
...
namespace CoverArtArchive
{
	...
	class CRedirect: public CExceptionBase
	{
	public:
			CRedirect(const std::string& Location)
			: CExceptionBase(Location,"Redirect"),
				m_Location(Location)
			{
			}

			virtual ~CRedirect() throw() {}
	...
	public:
		 ...
		 * @param FollowRedirects true to follow any redirects, false to return them
		 *
	...
	private:
		...
		int DoRequest(const std::string& URL);
